...
(defun ama-create-c++-imp (cname)
  ...
  (let ((old-buffer (current-buffer))
        (txt nil))
    (with-temp-buffer
      (insert-file "/homedirs/arthurma/emacs/template/c++-imp-header.txt")
...
(defun ama-create-c++-imp-class (cname)
  ...
  (let ((old-buffer (current-buffer))
        (txt nil))
    (with-temp-buffer
      (insert-file "/homedirs/arthurma/emacs/template/c++-imp-class.txt")
      (replace-regexp "RoutineGUICanvas" cname)
      (setq txt (buffer-substring-no-properties (point-min) (point-max))))
    (insert txt)))
...
(defun ama-c++-create-header ()
  ...
  (let (name txt)
    ...
    (setq txt (make-c++-header "/home/ama/work/elisp/c++-h\
eader-template.txt" name))
    (insert-string txt)))
